### Нстройка сетевого взаимодействия между виртуальным хостами в среде виртуализации Qemu-KVM с помощью инструментов _Vagrant_ + _Libvirt_
...
Все указанные сети имеют маску _24_ и содержат диапазон из _256_ _IP_-адресов, один из которых - адрес самой сети и один широковещательный адрес. Итого фактическая ёмкость сети - _254_ адреса. 
> [!NOTE]
> При создании виртуальных машин, _Vagrant_, в случае наличия виртуальной сети, имя которой содержится в описании машины, будет использовать её без изменения ранее заданных для неё параметров.
...
```
> [!WARNING]
> Во всех вышеуказанных примерах создаются либо сети _NAT_, либо обычные, но не изолированные от хоста, сети. А так как в нашей схеме у некоторых сетевых адаптеров в виртуальных хостах 
> заданы первые IP-адреса в виртуальных сетях, то мы получим конфликт IP-адресов между виртуальными машинами и хостом, который автоматически получает первый IP-адрес 
> на созданном интерфейсе в каждой неизолированной виртуальной сети. 
_Vagrant_ так же предупреждает об этом при выполнении операций над виртуальными машинами, такими, как _up_, _destroy_ и пр.:
```
...
Использование ключа _veryisolated_ для параметра _libvirt\_\_forward\_mode_ в следующем примере описания сети в Vagrantfile позволяет избежать подобных конфликтов, 
так как в таком случае создаётся изолированная виртуальная сеть:
```
...
```
На всех виртуальных машинах, кроме _inetRouter_, изменим маршрут по умолчанию, который изначально, при создании и запуске машины, указывает на _ip_-адрес хоста в сети управления. 
...
Добавленные нами правило для _Iptables_ и маршруты сохранятся до первой перезагрузки сервера или перезапуска сетевых интерфейсов. Следующие шаги позволят сохранить выполненные настройки.

##### Правила Iptables

Для сохранения правил _Iptables_ воспользуемся пакетом _nftables-persistent_. За его установку и сохранения загруженных правил отвечает следующий блок файла _Vagrantfile_ в секции _inetRouter_:
```
...
Изменённые или добавленные маршруты сохраним с помощью команды `ip route save > /etc/my-routes` в _Vagrantfile_, а активацию - с помощью строки `up ip route restore < /etc/my-routes`, помещённой в файл _/etc/network/interfaces_ 
