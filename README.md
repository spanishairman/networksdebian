...
```
> [!IMPORTANT]
> У данного подхода есть один недостаток, а именно - виртуальные сети должны быть созданы заранее, например, с помощью утилиты _virsh_, как было показано ранее. 
> Более того, несмотря на то, что при настройке провайдера _Vagrant Libvirt_ существует возможность установить параметр `:libvirt__always_destroy => 'false'`, 
> не позволяющий доменам, которые используют, но не создают сеть, уничтожать ее при уничтожении домена, при выполнении команды `Vagrant destroy` виртуальные сети уничтожаются вместе с мшинами, которые к ним подключены.
